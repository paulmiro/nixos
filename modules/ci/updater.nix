      ...
      packages.update-pipelines =
        let
          ...
          machines = builtins.attrNames (
            lib.filterAttrs (name: config: config.config.paul.ci.enable) self.nixosConfigurations
          );

          systemFor = name: self.nixosConfigurations.${name}.config.nixpkgs.hostPlatform.system;
